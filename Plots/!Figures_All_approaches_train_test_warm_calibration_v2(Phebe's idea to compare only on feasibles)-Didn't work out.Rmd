...
I group by fairness bound and take avg across 5 splits
...
# Readine FlowOCT Results (The warm_start version)
...
Let's only keep those instances that are feasible
```{r echo=TRUE, warning=FALSE}
keep_feasible <- function(data){
  tmp <- data
  tmp$feasibility <- tmp$sp.cal.pred <= tmp$fair.bound

  
  print('#Number of fair bounds across all samples with infeasible depth')
  print(summary(tmp$feasibility))

  tmp <- subset(tmp, tmp$feasibility)
  tmp$feasibility <- NULL
...
data_flow <- keep_feasible(data_flow)
data_kamiran_1 <- keep_feasible(data_kamiran_1)
data_kamiran_2 <- keep_feasible(data_kamiran_2)
data_kamiran_3 <- keep_feasible(data_kamiran_3)


data_flow$ID <- as.numeric(as.character(data_flow$sample))*10 + data_flow$fair.bound
data_kamiran_1$ID <- as.numeric(as.character(data_kamiran_1$sample))*10 + data_kamiran_1$fair.bound
data_kamiran_2$ID <- as.numeric(as.character(data_kamiran_2$sample))*10 + data_kamiran_2$fair.bound
data_kamiran_3$ID <- as.numeric(as.character(data_kamiran_3$sample))*10 + data_kamiran_3$fair.bound

#Now we save samples and fairbounds that are feasible in all approaches
intersect_IDs<- Reduce(intersect,list(data_flow$ID,data_kamiran_1$ID,data_kamiran_2$ID,data_kamiran_3$ID))
data_flow <- data_flow[data_flow$ID %in% intersect_IDs,]
data_kamiran_1 <- data_kamiran_1[data_kamiran_1$ID %in% intersect_IDs,]
data_kamiran_2 <- data_kamiran_2[data_kamiran_2$ID %in% intersect_IDs,]
data_kamiran_3 <- data_kamiran_3[data_kamiran_3$ID %in% intersect_IDs,]
...
data$approach<- factor(data$approach, levels = c('FairOCT','(IGC).Relab','(IGC-IGS).Relab','(IGC+IGS).Relab'))
```
